...
# Fully self-contained: pulls the Top 100 Restoration Shaman rankings for every SSC/TK boss,
# then pulls the healing table for every one of those 1000 parses.
#
...
#
# If your layout differs, just change $classesRoot below to match.
#
# Creates:  data\Classes\Shaman\{date}\rankings_hydross.json  (etc.) - if not already present
# Writes:   data\Classes\Shaman\{date}\{BossName}\{reportID}_{fightID}_{playerName}.json
#
