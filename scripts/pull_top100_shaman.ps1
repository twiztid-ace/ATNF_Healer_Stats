...
#
# v2 GraphQL pull, modeled 1:1 on pull_top100_druid.ps1 (see that file's header
# and C:\Users\raymo\.claude\plans\playful-baking-sunset.md's "Phase 3" plan for
# the full rationale). This is a from-scratch rewrite, not a patch of the old
# pull_top100_shaman.ps1 - the old script called the v1 REST API, wrote the old
# date-stamped-folder convention, and pulled only a single 5-entry-truncated
# healing TABLE per parse (no casts/consumables/activetime/deaths at all). The
# old script is preserved as pull_top100_shaman_v1.ps1 for reference/rollback.
#
# Because the old script's on-disk data has no events-shaped files at all, there
# was nothing to migrate forward via migrate_class_to_active.ps1 (that tool
# expects *_healing_events.json files to move). This script instead bootstraps a
# brand-new empty active\/archived\/manifest.json from scratch on its first run -
# every boss starts with lastPulledDate = null, which the diff logic below
# already treats as "first pull, fetch everything." The old
# data\Classes\Shaman\2026-07-10\ date folder is left untouched on disk.
#
# CLASS-SPECIFIC RESEARCH (done against a real Vajomee report, Z4zNt28raQ6GLbkC,
# 9 real boss kills, before writing this script - see the plan file's "Discovery
# pass" section):
#   - Real Resto Shaman healing-cooldown candidates confirmed from real cast
#     data: Earth Shield (guid 32594, applied to specific players - tank most of
#     the night, with real target swaps - tracked with self-vs-other targeting
#     the same way Druid's Swiftmend is), Mana Tide Totem (guid 16190, totem
#     placement - targetless, ~once/fight cadence), Ancestral Swiftness (guid
#     16188, the Totem of Ancestral Swiftness trinket-summoned totem - also
#     targetless, ~once/fight cadence).
#   - No Rebirth-equivalent exists for Shaman in this TBC ruleset - confirmed
#     absent from all 9 real boss kills despite real deaths occurring in them.
#     That concept is fully omitted here and from the Shaman template, not
#     force-mapped or left as a permanent zero row.
#   - No Tree-of-Life-equivalent exists worth tracking - Water Shield (guid
#     33736) appeared in only 3 of 9 fights with no sustained-uptime pattern
#     (looks like periodic maintenance recast, not a toggled form), so the
#     interval-reconstruction machinery Druid's script has for Tree of Life
#     (guid 33891) is dropped entirely here rather than forced onto a weak
#     analog. The consumables.json this script writes has no
#     treeOfLifeUptimePct-equivalent field at all; build_boss_report_data.ps1's
#     TreeOfLifePct passthrough already reads this field defensively (falls
#     through to $null if absent), so no changes were needed there for this.
#   - Real mana-potion item name confirmed as "Restore Mana" (guid 28499) - same
#     value already used for Druid, so no new manaPotionNameByClass value was
#     needed, just a new class key pointing at the same string.
#   - `characterRankings(className: "Shaman", specName: "Restoration", ...)`
#     confirmed live against real Karathress data before this script was
#     written (100 real entries returned, each carrying class:"Shaman",
#     spec:"Restoration") - not assumed from the old v1 script's classID/specID
#     values.
#
...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File pull_top100_shaman.ps1
#   powershell -ExecutionPolicy Bypass -File pull_top100_shaman.ps1 -MaxThreads 5
...
# boss name -> (rankings filename, encounter ID) - shared across every
...
# ===== Manifest load/save - identical to pull_top100_druid.ps1 ===== =====
...
# pull_top100_druid.ps1: everything it needs is passed in as an argument,
# including the module's absolute path (a RunspacePool worker does NOT inherit
# the parent session's Import-Module). No Tree-of-Life-equivalent buff-uptime
# logic here - see this file's header for why that was dropped for Shaman. =====
$workerScript = {
    param(
    ...
    # per-worker local hashtable (built directly with this fix from day one -
    # see pull_top100_druid.ps1's header for the real-data finding that
    # motivated it: at Top-100 scale, a per-worker cache means every parse
    # re-resolves the same handful of common ability guids via its own
    # gameData.ability() call, a real and significant chunk of unnecessary API
    # traffic). This same per-worker pattern is intentionally left alone in
    # pull_character_TEMPLATE.ps1, where only ~9-10 workers run per character
    # pull and the redundant-call cost is genuinely negligible at that scale.
    ...
    # (Water Shield showed no sustained/toggled uptime pattern worth this
    # machinery). Shaman's consumables.json simply has no such field.
    ...
    # active time - table(dataType: Healing, sourceClass: Shaman) - same v2
    # table call as Druid's script, already parameterized on $className.
    ...
    if (-not (Test-Path $deathsOutFile)) {
        ...
        # claimer succeeded. Same fix applied to pull_top100_druid.ps1 - see
        # WORKFLOW.md gotcha #24 for the "a full re-run picks it up fresh"
...
foreach ($bossName in $bosses.Keys) {
    ...
    # this script was written: characterRankings(className: "Shaman",
    # specName: "Restoration", ...) returns 100 real entries, each carrying
    # class:"Shaman", spec:"Restoration". NOT written to disk yet - need to
    ...
    # plus reads .duration for HPS - same reshape Druid's script does).
