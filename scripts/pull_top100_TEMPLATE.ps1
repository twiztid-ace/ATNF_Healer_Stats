# pull_top100_TEMPLATE.ps1
#
# GENERIC TEMPLATE - copy this file, rename to pull_top100_{class}.ps1, and edit
# ONLY the four lines in the "CLASS CONFIG" block below. Nothing else needs to change.
#
# Fully self-contained: pulls the Top 100 rankings (for the given class/spec) for every
...
#
# Creates:  data\Classes\{Class}\{date}\rankings_hydross.json  (etc.) - if not already present
# Writes:   data\Classes\{Class}\{date}\{BossName}\{reportID}_{fightID}_{playerName}.json
#
# Usage: powershell -ExecutionPolicy Bypass -File pull_top100_{class}.ps1

# ============ CLASS CONFIG - EDIT THESE FOUR LINES FOR A NEW CLASS ============
$className = "Druid"      # Must match the WCL sourceclass name exactly (Druid, Shaman, Priest, Paladin, ...)
$classID = 2               # From GET /classes - e.g. Druid=2, Paladin=6, Priest=7, Shaman=9
$specID = 4                # Healing spec ID within that class - e.g. Druid Restoration=4, Paladin Holy=1,
                            #   Priest Discipline=1 or Holy=2, Shaman Restoration=3
